...
- **About（README）**: 「Markmiru について...」（`menu:about`）→ `README.md`（概要・機能一覧等）を `//go:embed` で埋め込み、`ReadReadme()` で取得 → `openReadme()`。`ReadReadme()` は末尾に**バージョン（git ショート SHA）**を追記する（ldflags `-X main.version` で埋め込み。素のビルドでは `dev`）。OS のプロパティにも同じ版を埋め込む（AGENTS.md「バージョン番号」参照）。
