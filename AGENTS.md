...
- 信頼できない Markdown を開いても安全なよう防御を行う（HTML サニタイズ・CSP・外部リンクは OS ブラウザへ委譲・外部画像はファイルごとに表示確認）
...
| セキュリティ | **CSP**（本番ビルドのみ注入） | `script-src 'self'` 等。外部リンクは `BrowserOpenURL` で OS ブラウザへ委譲 |
